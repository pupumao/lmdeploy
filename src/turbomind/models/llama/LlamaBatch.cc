...
void LlamaBatch<T>::AllocateBuffer(size_t batch_size, size_t session_len, int cache_block_seq_len)
{
    ...
    context_decoder_quant_buf_ =
        (T*)allocator_->reMalloc(context_decoder_quant_buf_, sizeof(T) * max_context_token_num_ * hidden_units, false);
...
void LlamaBatch<T>::FreeBuffer()
{
    ...
    if (is_allocate_buffer_) {
        ...
        allocator_->free((void**)&context_decoder_quant_buf_);
...
bool LlamaBatch<T>::Forward(GenerationState& g, int iter)
{
    ...
    for (int p = 0; p < (int)offsets.size() - 1; ++p) {
        ...
        model_->forwardUnified(decoder_output_buf_ + first * model_->hidden_units_,
                               ...
                               context_decoder_quant_buf_,  // add for act quant
                               (void**)block_ptrs_,
